...
def admin_dashboard(request):
    ...
    if latest:
        ...
        thresholds = {
            ...
            'gas_resistance': {'warning': (None, 35), 'alert': (None, 20)},
...
def admin_dashboard_new(request):
    """View for the new admin dashboard with alert notifications"""
...
def ble_bridge(request):
    """View for the BLE Bridge interface."""
    # Get the latest reading
    latest = SensorReading.objects.order_by('-timestamp').first()
    ...
    # Prepare sensor data with status and changes
    sensor_data = {
...
def ble_bridge_modal(request):
    """View for the BLE Bridge interface with modal alerts."""
    # Get the latest reading
    latest = SensorReading.objects.order_by('-timestamp').first()
    ...
    return render(request, 'main/ble_bridge_modal.html', context)
