...
const sensorSuggestions = {
    ...
    gas_resistance: {
        warning: {
            high: [
                "Air quality is below optimal levels",
                "Ensure proper ventilation in the area",
                "Consider using air purifiers if available",
                "Monitor for potential sources of indoor air pollution"
            ],
            low: [
                "Significant air quality degradation detected (below 50 kΩ)",
        ...
        alert: {
            high: [
                "IMMEDIATE ACTION REQUIRED: Critical air quality issue detected",
                "Increase ventilation immediately",
                ...
                "Contact facility management for immediate assistance"
            ],
            low: [
                "IMMEDIATE ACTION REQUIRED: Severe air quality degradation (below 20 kΩ)",
...
function getSensorSuggestions(sensorType, status, value) {
    ...
    const thresholds = {
        ...
        'gas_resistance': { 'high': null, 'low': 35 },
...
// Update the updateSensorCard function
function updateSensorCard(sensorName, data) {
    ...
    const changeElement = card.querySelector('.sensor-change');
    const changePercentElement = card.querySelector('.sensor-change-percent');
    ...
    if (statusElement) {
        ...
        statusElement.className = `sensor-status status-${data.status.toLowerCase()}`;
    }
    if (changeElement) {
    ...
    // Check for status changes and show modal
    if (data.status === 'Alert' || data.status === 'Warning') {
        const lastState = window.lastAlertState[sensorName];
        if (!lastState || lastState.status !== data.status) {
            const message = `${data.display_name} is in ${data.status} state. Current value: ${formatSensorValue(sensorName, data.value)}`;
            if (typeof window.showAlertModal === 'function') {
                window.showAlertModal(data.status, message);
            }
        }
    }
    
    // Update last alert state
    window.lastAlertState[sensorName] = {
        status: data.status,
        value: data.value
    };
} 